        ...
        <main class="main dd">
                    ...
                    <div class="tg">
                        ...
                        <div class="ne">
                            ...
                            <form method="post" action="func_account.php" id="vid" onchange="document.querySelector('#vid').submit()">
                                <?php
                                    if(isset($_SESSION['vid2'])) {
                                        ...
                                        $sql = mysqli_query($connect, "SELECT * FROM kind_of_sport WHERE id_kind_of_sport NOT IN ('$id_vid2')");
                                        echo'
                                        ...
                                        $sql = mysqli_query($connect, "SELECT * FROM kind_of_sport WHERE id_kind_of_sport");
                                        echo'
                                        <select form="vid" name="vid">
                                        <option value="-1">Выбор вида спорта</option>';
                                        while($vid = mysqli_fetch_object($sql)) {
                            ...
                            <div id="reviews" class="reviews">
                                <form method="post" action="func_account2.php" id="vid2" onchange="document.querySelector('#vid2').submit()">
                                    <?php 
                                    $sql = mysqli_query($connect, "SELECT * FROM kind_of_sport WHERE id_kind_of_sport NOT IN ('$id_vid')");
                                    echo'
                                    <select form="vid2" name="vid2">
                                    <option value="-1">Выбор вида спорта</option>';
                                    while($vid2 = mysqli_fetch_object($sql)) {
                                        $id_vid2 = $_SESSION['vid2'];
                                        if (isset($id_vid2) & $vid2->id_kind_of_sport == $id_vid2){
                                        echo '
                                        <option selected name="'. $vid2->id_kind_of_sport .'" value=" ' . $vid2->id_kind_of_sport . ' "> ' . $vid2->name_kind_of_sport . '</option>
                                    ';}
                                    else {
                                        echo '
                                        <option name="'. $vid2->id_kind_of_sport .'" value=" ' . $vid2->id_kind_of_sport . ' "> ' . $vid2->name_kind_of_sport . '</option>
                                    ';
                                    }
                                    }?>
                                    </select><br><br>
                                    <?php
                                    $sql = mysqli_query($connect, "SELECT id_trainer, CONCAT (trainer.surname_trainer, ' ', trainer.name_trainer) AS fio_trainer FROM `trainer` INNER JOIN `kind_of_sport` ON trainer.id_kind_of_sport = kind_of_sport.id_kind_of_sport WHERE kind_of_sport.id_kind_of_sport ='$id_vid2'");
                                    echo '<select form="vid2" name="trainer2">
                                    <option value="-1">Выбор тренера</option>';
                                    while($trainer2 = mysqli_fetch_object($sql)) {
                                        $id_trainer2= $_SESSION['trainer2'];
                                        if (isset($id_trainer2) & $trainer2->id_trainer == $id_trainer2){
                                        echo '
                                        <option selected name="'. $trainer2->id_trainer .'" value=" ' . $trainer2->id_trainer . ' "> ' . $trainer2->fio_trainer . '</option>
                                    ';}
                                    else {
                                    echo '
                                        <option name="'. $trainer2->id_trainer .'" value=" ' . $trainer2->id_trainer . ' "> ' . $trainer2->fio_trainer . '</option>
                                    ';}
                                    }?>
                                        </select><br><br>
                                </form>
                                <a href="account.php" class="ne_a">− Убрать вид спорта</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            if (isset($_SESSION['trainer2']) & isset($_SESSION['vid2'])):
                if (isset($_SESSION['trainer2']) & $_SESSION['vid2'] >= 0 & $_SESSION['trainer2'] >= 0):
                    $id_trainer2 = $_SESSION['trainer2'];
                    $id_user = $_SESSION['id_yamas_user'];
                    $sql = mysqli_query($connect, "SELECT * FROM training WHERE id_yamas_user = '$id_user' AND id_trainer = '$id_trainer2'");
                    $trainer2 = mysqli_fetch_object($sql);
                    $sql = mysqli_query($connect, "SELECT name_kind_of_sport FROM `kind_of_sport` WHERE id_kind_of_sport = '$id_vid2'");
                    $vid2 = mysqli_fetch_object($sql);

                    $sql = mysqli_query($connect, "SET lc_time_names = 'ru_RU'");
                    $sql = mysqli_query($connect, "SELECT DATE_FORMAT(date_time_start,'%W, %e %M %Y') AS `date`, DATE_FORMAT(date_time_start,'%H:%m') AS `time_start`, DATE_FORMAT(date_time_end,'%H:%m') AS `time_end` FROM training WHERE id_yamas_user = '$id_user' AND id_trainer = '$id_trainer2'");
                    if (isset($trainer2)):
                         while($datetime = mysqli_fetch_assoc($sql)):?>
                                <div class="block rs">
                                    <h1><?=$vid2->name_kind_of_sport?></h1>
                                    <h3> c <?= $datetime['time_start']?> </h3>
                                    <h3>до <?= $datetime['time_end']?></h3>
                                    <h2> <?= $datetime['date']?></h2>
                                </div>
